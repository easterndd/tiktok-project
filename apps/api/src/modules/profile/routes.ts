...
export async function registerProfileRoutes(app: FastifyInstance) {
  ...
  app.get('/me/history', { preHandler: requireUser }, async (request) => {
    ...
    return { items: records.map((record) => { const albumTranslation = translated(record.episode.album.translations, locale); const episodeTranslation = translated(record.episode.translations, locale); return { id: record.episode.album.id, title: albumTranslation?.title ?? record.episode.album.title, description: albumTranslation?.description ?? record.episode.album.description, coverUrl: albumTranslation?.coverUrl ?? record.episode.album.coverUrl, episodeCount: record.episode.album._count.episodes, updatedAt: record.episode.album.updatedAt.toISOString(), episodeId: record.episodeId, episodeNo: record.episode.episodeNo, episodeTitle: episodeTranslation?.title ?? record.episode.title, progress: record.durationMs ? Math.min(record.positionMs / record.durationMs, 1) : 0, resumePositionMs: record.positionMs, durationMs: record.durationMs, watchedAt: record.updatedAt.toISOString() }; }) };
  });
  app.get('/me/favorites', { preHandler: requireUser }, async (request) => {
    ...
    return { items: records.map(({ album }) => { const albumTranslation = translated(album.translations, locale); return { id: album.id, title: albumTranslation?.title ?? album.title, description: albumTranslation?.description ?? album.description, coverUrl: albumTranslation?.coverUrl ?? album.coverUrl, episodeCount: album._count.episodes, updatedAt: album.updatedAt.toISOString(), language: album.language, status: 'ONLINE' as const }; }), nextCursor: null };
  });
}
