...
export async function registerInteractionRoutes(app: FastifyInstance) {
  app.put('/albums/:albumId/like', { preHandler: requireUser }, async (request) => {
    ...
    const album = await app.prisma.album.findFirst({ where: { id: albumId, ...publicAlbumWhere(app.config) }, select: { id: true, regions: true } });
    if (!album || !isAlbumVisibleInCountry(album.regions, requestCountry(request, app.config.TRUST_GEO_COUNTRY_HEADER))) throw Object.assign(new Error('Album not found.'), { statusCode: 404 });
  ...
  app.post('/albums/:albumId/share', async (request) => {
    ...
    const album = await app.prisma.album.findFirst({ where: { id: albumId, ...publicAlbumWhere(app.config) } });
    if (!album || !isAlbumVisibleInCountry(album.regions, requestCountry(request, app.config.TRUST_GEO_COUNTRY_HEADER))) throw Object.assign(new Error('Album not found.'), { statusCode: 404 });
