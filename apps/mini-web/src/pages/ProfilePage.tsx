import type { CursorPage, HistoryItem, Preferences, UserProfile } from '@breezereels/shared-types';
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { ArrowLeft, Bookmark, Check, ChevronRight, Clock3, Globe2, LogIn, Play, Settings2, UserRound } from 'lucide-react';
...
export function ProfilePage() {
  ...
  return <section className={styles.page}>
    ...
    <section className={styles.section}><div className={styles.sectionHeading}><h2><Settings2 size={18} aria-hidden="true" /> {t(locale, 'settings')}</h2></div><div className={styles.settingRow}><span><Globe2 size={17} aria-hidden="true" /><span><strong>{t(locale, 'language')}</strong><small>{supportedLocales.find((item) => item.value === selectedLocale)?.label}</small></span></span><select value={selectedLocale} onChange={(event) => { const next = event.target.value as Preferences['locale']; setSelectedLocale(next); setStoredLocale(next); void savePreferences.mutateAsync({ locale: next }); }} aria-label={t(locale, 'language')}>{supportedLocales.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></div><label className={styles.settingRow}><span><Check size={17} aria-hidden="true" /><span><strong>{t(locale, 'autoplay')}</strong><small>{t(locale, 'continueWithoutTapping')}</small></span></span><input type="checkbox" checked={preferences.data?.autoplay ?? true} onChange={(event) => savePreferences.mutate({ autoplay: event.target.checked })} /></label><label className={styles.settingRow}><span><Check size={17} aria-hidden="true" /><span><strong>{t(locale, 'reducedData')}</strong><small>{t(locale, 'useLessData')}</small></span></span><input type="checkbox" checked={preferences.data?.reducedData ?? false} onChange={(event) => savePreferences.mutate({ reducedData: event.target.checked })} /></label></section>
  </section>;
}
