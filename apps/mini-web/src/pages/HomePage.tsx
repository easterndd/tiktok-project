import type { AlbumSummary, HomeResponse } from '@breezereels/shared-types';
...
function AlbumRail({ title, items, locale }: { title: string; items: AlbumSummary[]; locale: import('@breezereels/shared-types').Locale }) {
...
export function HomePage() {
  ...
  return <div className={styles.page}>
    ...
    {componentIsEnabled(ui, 'HOME_INTRO') && <header className={styles.intro}><div><p className="eyebrow">QUICK REELS</p><h2>{t(locale, 'discover')}</h2><p>{t(locale, 'discoverCopy')}</p></div><Link to="/search" className={styles.roundButton} aria-label={t(locale, 'search')}><ArrowRight size={19} /></Link></header>}
