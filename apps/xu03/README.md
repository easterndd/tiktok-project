...
- `VITE_TIKTOK_CLIENT_KEY`: the `xu03` TikTok Client Key.
- ad placement IDs created for `xu03`.
- `minis.config.json.appId`: the numeric App ID issued by TikTok for `xu03`.
...
Before TikTok review, verify the privacy policy and terms in `apps/mini-web/src/pages/legal-docs.ts` against the actual operator, contact details, hosting, and governing law for `xu03`. The displayed product name is changed to `xu03` by this build; the legal facts are not changed automatically.
